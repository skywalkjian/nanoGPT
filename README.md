...
目前仓库里已经保留了 FAR 路径，但还没有形成和 BAR 同等粒度的对照证据。
...
三条路径在同一训练预算下的结果，就能更清楚地回答：

- block 级压缩是否损失了关键收益
- full attention residuals 是否在小规模实验中更有效
- BAR 的优势来自 residual attention 本身，还是来自 blockwise 结构约束
